...
1. Public client resolves `journal.chaseconover.com` via standard internet DNS → Cloudflare DNS → Cloudflare edge IP.
2. Public client TLS-handshakes with Cloudflare's edge (Cloudflare presents the cert).
3. Cloudflare's edge forwards the request through the open `cloudflared` tunnel to the Pi.
4. `cloudflared` looks up the hostname in its dashboard-configured ingress rules → forwards to `http://chronicle:3000` on the internal Docker network.

**Tailnet clients hitting `journal.chaseconover.com`** also resolve to the Cloudflare edge (Pi-hole has no record for the public host, so the query forwards upstream). This means tailnet traffic to Chronicle takes the public path through Cloudflare, not the tailnet path. If you want fast direct-to-Pi access from the tailnet, add a `proxy_hosts: [journal.lab.chaseconover.com]` entry in the chronicle service config and route there from a tailnet device — keeps the public hostname for friends and a tailnet-only hostname for fast local access.

See [cloudflare-tunnel.md](cloudflare-tunnel.md) for the full picture.
